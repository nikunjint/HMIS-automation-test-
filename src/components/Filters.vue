<template>
  ...
  <form ref="form" class="row g-1" @submit.prevent="OnCreatePostTwo">
      <div class="card mb-1">
        <div class="card-body">
          <div class="row">
              ...
              <div>
                <div class="input-group input-group-sm">
                  <span class="input-group-text x" id="inputGroup-sizing-sm"
                    >From Date</span
                  >
   <input type="text" class="nepali-datepicker form-control" :value="nepalidates.bs" @input="updateNepaliDates" placeholder="MM/DD/YYYY" />
                </div>
              ...
              <div>
                <div class="input-group input-group-sm">
                  <span class="input-group-text x" id="inputGroup-sizing-sm"
                    >To Date:</span
                  >
               <input type="text" class="nepali-datepickertwo form-control" :value="nepalidatestwo.bs" @input="updateNepaliDatesTwo" placeholder="MM/DD/YYYY" />
              ...
              <div>
                <input class="form-control" type="number" v-model="numDays"  placeholder="Days" />
              </div>
      ...
      <option value="LABNO">LAB NUMBER</option>
            ...
            <button type="submit"
              ...
              class="btn btn-filled addnew-btn float-end"
...
<div class="form-check">
  ...
  v-model="selectedValue"
    ...
    id="flexRadioDefault1" @change="OnCreatePostTwo"
    checked
  />
...
<div class="form-check"> 
  <input
    class="form-check-input"
      type="radio"     value="notreceived"
  v-model="selectedValue"
    name="flexRadioDefault"
    id="flexRadioDefault2" @change="OnCreatePostTwo"
  />
...
<div class="form-check">
  <input
    class="form-check-input"
    type="radio"     value="3"
  v-model="selectedValue" @change="OnCreatePostTwo"
  ...
  <div v-if="selectedOption === 'option1'" >
        ...
        >
          <template #item-TESTPROGRESSSTATUS="item">
            ...
            <div class="status" v-html="item.TESTPROGRESSSTATUS == 2 ? '<div class=Pendingheader>Pending</div>' : ''"></div>  
            <div class="status" v-html="item.TESTPROGRESSSTATUS == 3 && item.ISLABRECEIVED == 'Y' ? '<div class=lreceieved>Lab Received</div>' : ''"></div>  
            <div class="status" v-html="item.TESTPROGRESSSTATUS == 3 && item.ISLABRECEIVED == 'N' ? '<div class=scollect>Sample Collected</div>' : ''"></div>           
            {{ item.TESTPROGRESSSTATUS == 4 ? "Findings Entered" : "" }}
            {{ item.TESTPROGRESSSTATUS == 5 ? "Verified" : "" }}
            <div class="status" v-html="item.TESTPROGRESSSTATUS == 6 ? '<div class=Dispatchedheader>Dispatched</div>' : ''"></div> 
...
export default {
 ...
 note: '',
    ...
    nepalidatestwo: {
      ...
      headers: [
        { text: "Patient Id", value: "PATIENTTESTID" }, 
  ...
  mounted() {
    ...
    this.OnCreatePostTwo();

      this.nepalidates.bs = "2079/09/12";
  this.nepalidatestwo.bs = "2079/09/12";
   ...
   mainInput.nepaliDatePicker({
    ...
    axios
      .post(
        ...
        { fromDate: "2079/09/12", toDate: "2079/09/12", labSection: "patientlist", status: "all", globalSearchBy: "LABNO", globalSearchValue: this.globalsearch },
        config
      )
      .then((res) => {
    ...
    methods: {
      ...
      this.nepalidates = event.target.value;
    },
  ...
  OnCreatePostTwo(){
      ...
      .post(
        `${import.meta.env.VITE_API_ENDPOINT}/lab/getPatientList`,
        { fromDate: this.nepalidates.bs, toDate: this.nepalidatestwo.bs, labSection: "patientlist", status: this.selectedValue, globalSearchBy: this.searchField, globalSearchValue: this.globalsearch }, {
            headers: {
        Authorization: `Bearer ${this.$store.state.userDetails.access_token}`,
      }},
      )
