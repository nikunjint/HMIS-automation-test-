        ...
        >
          <template #item-TESTPROGRESSSTATUS="item">
            ...
            /> -->
                 ...
                 <button
                @click="
                  labauthorize(item.PATIENTTESTID)
                "
                class="btn btn-sm btn-primary small authorize"  
              >
                Authorize
              </button>
  ...
  methods: {
    ...
    updateNepaliDatesTwo(event) {
      this.nepalidatestwo.bs = event.target.value;
        ...
        async labauthorize(PATIENTTESTID) {
      try {
        ...
        axios.post(
          `${import.meta.env.VITE_API_ENDPOINT}/lab/authorizeTest`,
          formData,
