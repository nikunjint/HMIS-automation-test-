<template>
  ...
  <div
    class="d-none"
    v-for="samplecollection in $store.state.labReceivedData.patientListData
      .result"
    v-bind:key="samplecollection"
  >
    <div class="d-flex">
      <div>{{ samplecollection.PATIENTTESTID }}</div>
      :
      <div>{{ samplecollection.ISLABRECEIVED }}</div>
    </div>
  </div>
      ...
      <div class="position-relative mt-2">
        ...
        >
          ...
          <template #item-indicator="item">
            <div v-if="item.ISLABRECEIVED == 'Y'">
              <button class="btn btn-sm btn-primary small">Hold Report</button>
            </div>
            <div
              class="d-flex align-items-center"
              v-if="item.TESTPROGRESSSTATUS == 3 && item.ISLABRECEIVED == 'N'"
            >
              <button
                @click="
                  labreceive(item.PATIENTID, item.SAMPLENO, item.PATIENTTESTID)
                "
                class="btn btn-sm btn-primary small"
              >
                Lab Receive
              </button>
              <button class="btn btn-sm btn-primary small ms-2">
                Hold Report
              </button>
            </div>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "TestID", value: "PATIENTTESTID" },
  ...
  methods: {
    ...
    async labreceive(PATIENTID, SAMPLENO, PATIENTTESTID) {
      try {
        const formData = new FormData();
        formData.append("patientId", PATIENTID);
        formData.append("sampleNo", SAMPLENO);
        formData.append("patientTestId[]", PATIENTTESTID);

        axios.post(
          `${import.meta.env.VITE_API_ENDPOINT}/lab/labReceiveTest`,
          formData,
        ...
        this.$store.dispatch("fetchLabReceivedData");
      } catch (err) {}
    },
