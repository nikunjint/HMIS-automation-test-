<template>
  ...
  <h5 class="mb-0">DEPARTMENT RECEIVE
</h5>
  <hr class="mt-2" />
  ...
  <div class="searchFilter p-2">
    <form ref="form" class="row g-1" @submit.prevent="OnCreatePost">
      ...
      <div
        ...
        "
      >
        <div class="position-absolute left-5">
          <div class="input-group input-group-sm">
            <span class="input-group-text x" id="inputGroup-sizing-sm"
              >Search</span
            >
        ...
        >
          <template #item-TESTPROGRESSSTATUS="item">
            ...
            <div
              class="status"
              v-html="
                item.ISDEPARTMENTRECEIVED == 'Y' && item.ISLABRECEIVED == 'Y'
                  ? '<div class=lreceieved>Department Received</div>'
                  : ''
              "
            ></div>
              ...
              class="status"
              v-html="
                item.ISDEPARTMENTRECEIVED == 'N' && item.ISLABRECEIVED == 'Y'
            ...
            ></div>
             ...
             <template #item-indicator="item">
                ...
                <form
                  @submit.prevent="
                    DepartmentReceived(item.PATIENTTESTID)
                  "
                >
...
export default {
  ...
  mounted() {
    ...
    this.selectedValue = "labreceived";
  ...
  methods: {
    ...
    updateNepaliDatesTwo(event) {
      this.nepalidatestwo.bs = event.target.value;
        ...
        async DepartmentReceived(PATIENTTESTID) {
      try {
        ...
        axios.post(
          `${import.meta.env.VITE_API_ENDPOINT}/lab/departmentReceive`,
          formData,
